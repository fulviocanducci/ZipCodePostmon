...
namespace Canducci.ZipCodePostmon
{
    public sealed class ZipCodeResult
    {
        ...
        public ZipCode Find(ZipCodeNumber number) => Find(number.Number);
        public ZipCode Find(string number)
        {
            Stream stream = ZipCodeRequest.GetStream(number);
        ...
        public async System.Threading.Tasks.Task<ZipCode> FindAsync(ZipCodeNumber number) => await FindAsync(number.Number);
        public async System.Threading.Tasks.Task<ZipCode> FindAsync(string number)
        {
            Stream stream = await ZipCodeRequest.GetStreamAsync(number);
            return GetJsonStreamSerializerToZipCode(stream);
        }
#endif
